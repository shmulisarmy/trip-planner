...
export function Event_C({
  ...
  const el: HTMLElement = (
    <div
      ...
      draggable="true"
      ondrag={() => {
        dragInfo.grabbing = { events_accesor_key, index };
        el.style.display = "none";
        hidden = el;
        drop_zone.style.height = `${duration * time_to_size_multiplier}px`;
        document.body.classList.add("durring-dragging");
      }}
      ondragover={() => {
        dragInfo.dragged_onto = { events_accesor_key, index };
        el.insertAdjacentElement("afterend", drop_zone);
        // drop_zone.parentElement.scrollTo({top: drop_zone.offsetHeight, behavior: "smooth"})
      }}
      ondragend={move_grabbed_data_to_right_spot}
      onDragEnd={() => {
        hidden.style.display = "block";
        drop_zone.parentElement?.removeChild(drop_zone);
        document.body.classList.remove("durring-dragging");
      }}
    >
      <div
        class="top"
      ...
      <div class="lower-text">
        ...
        <p style={{ "font-size": "16px" }}>
          ...
          <strong class="event-duration">
            <button
              onclick={() =>
                change_event_duration(events_accesor_key, index, -10)
              }
            >
              -
            </button>
  ...
  const resizeObserver = new ResizeObserver((entries) => {
    ...
    console.log(
      `index: ${saved_resize_event.index}, size: ${saved_resize_event.size}`
    );
  });
  if (resizeBox) {

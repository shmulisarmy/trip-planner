...
function swap() {
  ...
  if (dragInfo.grabbing_index != null && dragInfo.dragged_onto_index != null) {
    const temp = ev[dragInfo.grabbing_index];
    ev[dragInfo.grabbing_index] = ev[dragInfo.dragged_onto_index];
    ev[dragInfo.dragged_onto_index] = temp;
    setEvents([...ev]);
  } else {
...
function Event_C({
  ...
  end_time,
  index,
}: Event_ & { start_time: number; end_time: number; index: number }) {
  const event_image_size = !event_size_callapse ? "90px" : "40px";
  const event_style = event_size_callapse
    ? { height: `${Math.round(duration * time_to_size_multiplier)}px`, "border": "dashed 1px var(--color)" }
  ...
  return (
    <div
      ...
      style={event_style}
      draggable="true"
      ondrag={() => (dragInfo.grabbing_index = index)}
      ondragover={() => (dragInfo.dragged_onto_index = index)}
      ondragend={swap}
    >
      <div
        class="top"
      ...
      >
        ...
        <div
          class="text"
          style={{ "margin-left": "10px", "line-height": event_size_callapse? `6px`: "10px", "font-size": event_size_callapse? `14px`: "18px" }}
        >
          <h2>{name}</h2>
          <div class="time">
            <p>
      ...
      <p style={{ "font-size": "16px" }}>
              this event is {duration} minutes long
        </p>
      </div>
      <hr />
    </div>
...
function Events_C() {
  ...
  return (
    <>
      <button onclick={toggle_event_size_callapse}>toggle collapse</button>
      <div id="events">
